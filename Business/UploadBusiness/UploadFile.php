...
class UploadFile extends AbstractBusiness
{
     ...
     * @param null $relatively_path 文件存储相对路径,以public目录为根,不传默认upload
    ...
    public function upload($inputName, $relatively_path = null, $fileType = self::TYPE_IMAGE, $save_name = null)
    {
        ...
        try {
            $path = '/' . str_replace(DIRECTORY_SEPARATOR, '/', $relatively_path) . '/' . $file->getNameWithExtension();
            ...
            $data = array(
                ...
                'server_path' => self::getFileNetworkPath($this->container, $path)
            );
